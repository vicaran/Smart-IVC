...
var createCanvas = function () {
    ...
    var prepareBuildings = function (data, scene) {
        if (data[0]) {
            var coordinateZero = getFirstCoordinate(createRing(data[0].ringGlobalCoords));
            coordinateZero = latLngToXYZ(coordinateZero.Lat, coordinateZero.Lng);

            for (var i = 0; i < data.length; i++) {
                if (data[i]) {
                    var shape = createShape(data[i].ringGlobalCoords, coordinateZero);
                    var path = createPath(data[i].floors);
                    var firstCoord = getFirstCoordinate(createRing(data[i].ringGlobalCoords));
    ...
    var loadCity = function (scene) {
        $.ajax({
                   ...
                   // url: SERVER_URL + "building/city=1/", // ENTIRE CITY NEW
                   type: "GET",
                   success: function (data, textStatus, jqXHR) {
    ...
    var createScene = function () {
        ...
        // SET CAMERA
        var camera = new BABYLON.FreeCamera("FreeCamera", new BABYLON.Vector3(0, 100, -180), scene);
